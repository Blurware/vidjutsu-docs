...
`sourceVideoUrl` is optional and supplies textual source context only; this endpoint does not extract or attach an opening frame. The returned image is generated with a server-enforced no-overlay instruction. The endpoint is synchronous and shares the clone admission limit.
