...
```bash
vidjutsu clone check "https://cdn.vidjutsu.ai/imports/source.mp4"
```
...
Raw TikTok and Instagram post pages are rejected. Stage them first with the
download command or use `clone run`, which performs that step automatically.
...
`clone run` downloads the source video, checks its 15-second cap and cloneability, extracts frame `0`, composes that first frame with the character you passed, submits the Kling 3.0 clone, and polls for the final URL. It stops if the check verdict is weak, unless you pass `--force`.
...
vidjutsu clone starting-image \
  ...
  --first-frame "https://cdn.vidjutsu.ai/extracts/tt_8f2c.../frame-0.jpg"
```

| Flag | Required | Description |
|------|----------|-------------|
| `--character` | Yes | Stored `char_...` id |
| `--prompt` | Yes | Instructions for composing the starting frame |
| `--first-frame` | Yes | Public HTTPS first-frame image extracted from the staged source |
