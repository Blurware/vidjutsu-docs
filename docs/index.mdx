...
<CardGroup cols={2}>
  <Card title="Watch" icon="eye" href="/api-reference/watch/watch">
    Gemini watches your video with a freeform prompt. Ask any question about what it sees.
  </Card>
  <Card title="Extract" icon="scissors" href="/api-reference/extract/create">
    Pull frames, audio, and metadata from any video via ffmpeg.
  </Card>
...
1. **Get your API key** — `POST /v1/api_keys` with your email. Pay with any card via Stripe. Poll `GET /v1/api_keys/status` after payment.
2. **Analyze your content** — Call `/v1/watch`, `/v1/extract`, `/v1/transcribe`, or `/v1/check` on any public video URL.
3. **Store and organize** — Create accounts, posts, assets, and references as data records (0 credits). Tag them for filtering.
