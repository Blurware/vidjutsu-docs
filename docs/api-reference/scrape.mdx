...
Every scrape subcommand call counts as **one request** against the shared `scrape` daily rate limit (500/day), regardless of how many media files `download_media: true` stages. There is no per-file or per-credit charge — staging extra media does not consume additional quota beyond the single request.
